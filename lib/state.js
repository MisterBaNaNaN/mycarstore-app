...
function getOverloadThreshold() {
  const row = db.prepare('SELECT overload_threshold FROM site_state WHERE id = 1').get();
  return row.overload_threshold;
}
...
function getClients() {
  ...
  const npsStmt = db.prepare('SELECT * FROM nps_responses WHERE client_id = ? ORDER BY created_at DESC');
  ...
  return clientRows.map((c) => ({
    ...
    npsResponses: npsStmt.all(c.id).map(rowToNps)
  }));
}
...
function getDocTemplates() {
  return db.prepare('SELECT * FROM doc_templates ORDER BY nom ASC').all().map((t) => ({
    id: t.id,
    nom: t.nom,
    items: parseJsonSafe(t.items, [])
  }));
}
...
function getAdminState() {
  return {
    ...
    docTemplates: getDocTemplates(),
    overloadThreshold: getOverloadThreshold()
  };
}

/** Normalizes a phone number to bare digits with a leading 0 (so +33612345678 and 06 12 34 56 78 compare equal). */
