...
async function notifyAppointment(appt, kind) {
  ...
  logCommunication({ clientId: appt.client_id, appointmentId: appt.id, kind, channel: 'email', recipient: appt.email, result: email });
  logCommunication({ clientId: appt.client_id, appointmentId: appt.id, kind, channel: 'sms', recipient: appt.tel, result: sms });
  return { email, sms };
}

/** Sends a quick 1-5 satisfaction request by SMS once an invoice is marked paid; never throws. */
async function sendNpsRequest(client, invoice) {
  const body = `Bonjour ${client.nom},\n\nMerci pour votre visite chez MyCarStore ! Répondez à ce SMS avec une note de 1 (déçu) à 5 (excellent) pour nous dire comment ça s'est passé.\n\nMyCarStore — 03 65 67 69 62`;
  const sms = await sendSms({ to: client.tel, body });
  logCommunication({ clientId: client.id, invoiceId: invoice.id, kind: 'nps', channel: 'sms', recipient: client.tel, result: sms });
  return { sms };
}

function logCommunication({ clientId, appointmentId, invoiceId, kind, channel, recipient, result }) {
  let status = 'sent';
  if (result.skipped) status = 'skipped';
  else if (!result.ok) status = 'failed';
  try {
    db.prepare(`
      INSERT INTO communications (created_at, client_id, appointment_id, invoice_id, channel, kind, recipient, status, detail)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
...
module.exports = { sendEmail, sendSms, notifyAppointment, sendNpsRequest, toE164FR };
